...
from app.schemas.memory import MemoryCreate, MemoryUpdate, TagResponse
...
class TestMemoryRepository:
    ...
    async def test_list_all(self):
        ...
        async with session_factory() as session:
            repo = MemoryRepository(session)
            for i in range(5):
                await repo.create(MemoryCreate(content=f"Memory {i}", category="general"), user_id="user-1")
    ...
    async def test_list_all_filter_by_category(self):
        ...
        async with session_factory() as session:
            ...
            await repo.create(MemoryCreate(content="Profile", category="user_profile"), user_id="user-1")
    ...
    async def test_list_all_excludes_archived(self):
        ...
        async with session_factory() as session:
            repo = MemoryRepository(session)
            m1 = await repo.create(MemoryCreate(content="Active", category="general"), user_id="user-1")
            m2 = await repo.create(MemoryCreate(content="Archived", category="general"), user_id="user-1")
            m2.is_archived = True
            await session.flush()
    ...
    async def test_update(self):
        ...
        async with session_factory() as session:
            repo = MemoryRepository(session)
            memory = await repo.create(MemoryCreate(content="Original", category="general"), user_id="user-1")
    ...
    async def test_update_tags(self):
        ...
        async with session_factory() as session:
            repo = MemoryRepository(session)
            memory = await repo.create(MemoryCreate(content="Tag test", tags=["a"]), user_id="user-1")
    ...
    async def test_search_by_content_with_category(self):
        ...
        async with session_factory() as session:
            repo = MemoryRepository(session)
            await repo.create(MemoryCreate(content="Python rocks", category="general"), user_id="user-1")
            await repo.create(MemoryCreate(content="Python for ML", category="project"), user_id="user-1")
    ...
    async def test_get_by_category(self):
        ...
        async with session_factory() as session:
            repo = MemoryRepository(session)
            await repo.create(MemoryCreate(content="Pref: dark mode", category="preference"), user_id="user-1")
            await repo.create(MemoryCreate(content="Pref: long responses", category="preference"), user_id="user-1")
            await repo.create(MemoryCreate(content="General note", category="general"), user_id="user-1")
...
class TestMemoryService:
    ...
    async def test_create_memory(self):
        ...
        async with session_factory() as session:
            ...
            result = await service.create_memory(
                MemoryCreate(content="Hello", category="general"), user_id="user-1"
            )
    ...
    async def test_get_memory(self):
        ...
        async with session_factory() as session:
            ...
            created = await service.create_memory(
                MemoryCreate(content="Get me"), user_id="user-1"
            )
    ...
    async def test_update_memory(self):
        ...
        async with session_factory() as session:
            service = MemoryService(session)
            created = await service.create_memory(
                MemoryCreate(content="Original"), user_id="user-1"
            )
            updated = await service.update_memory(
                created.id, MemoryUpdate(content="Updated"), user_id="user-1"
            )
            assert updated is not None
            assert updated.content == "Updated"
    ...
    async def test_update_not_found(self):
        ...
        async with session_factory() as session:
            ...
            result = await service.update_memory(
                "nonexistent", MemoryUpdate(content="x"), user_id="user-1"
            )
            assert result is None
    ...
    async def test_delete_memory(self):
        ...
        async with session_factory() as session:
            service = MemoryService(session)
            created = await service.create_memory(
                MemoryCreate(content="Delete me"), user_id="user-1"
            )
        ...
        async with session_factory() as session:
            ...
            for i in range(5):
                await service.create_memory(
                    MemoryCreate(content=f"Item {i}", category="general"), user_id="user-1"
                )
    ...
    async def test_list_filter_by_category(self):
        ...
        async with session_factory() as session:
            service = MemoryService(session)
            await service.create_memory(MemoryCreate(content="General", category="general"), user_id="user-1")
            await service.create_memory(MemoryCreate(content="Pref", category="preference"), user_id="user-1")
    ...
    async def test_get_profile_memory(self):
        ...
        async with session_factory() as session:
            service = MemoryService(session)
            await service.create_memory(
                MemoryCreate(content='{"name":"Alice"}', category="user_profile"), user_id="user-1"
            )
    ...
    async def test_get_preferences(self):
        ...
        async with session_factory() as session:
            service = MemoryService(session)
            await service.create_memory(
                MemoryCreate(content="Dark mode", category="preference"), user_id="user-1"
            )
            await service.create_memory(
                MemoryCreate(content="Short answers", category="preference"), user_id="user-1"
            )
    ...
    async def test_get_project_memories(self):
        ...
        async with session_factory() as session:
            service = MemoryService(session)
            await service.create_memory(
                MemoryCreate(content="Project Alpha", category="project"), user_id="user-1"
            )
    ...
    async def test_archive_and_restore(self):
        ...
        async with session_factory() as session:
            service = MemoryService(session)
            created = await service.create_memory(MemoryCreate(content="Archivable"), user_id="user-1")
    ...
    async def test_get_context_for_query(self):
        ...
        async with session_factory() as session:
            service = MemoryService(session)
            await service.create_memory(
                MemoryCreate(content="User likes Python programming"), user_id="user-1"
            )
    ...
    async def test_record_conversation_memory(self):
        ...
        async with session_factory() as session:
            ...
            result = await service.record_conversation_memory(
                "Discussed project timeline", user_id="user-1", importance=0.7
            )
    ...
    async def test_get_stats(self):
        ...
        async with session_factory() as session:
            service = MemoryService(session)
            await service.create_memory(MemoryCreate(content="A", category="general"), user_id="user-1")
            await service.create_memory(MemoryCreate(content="B", category="project"), user_id="user-1")
            await service.create_memory(MemoryCreate(content="C", category="project"), user_id="user-1")
...
class TestMemoryAPI:
    ...
    async def test_list_filter_by_category(self, client, auth_headers):
        ...
        response = await client.get(
            "/api/v1/memory?category=user_profile", headers=auth_headers
        )
        assert response.status_code == 200
        data = response.json()
        assert data["total"] >= 1
    ...
    async def test_user_isolation_api(self, client):
        # Register two users
        r1 = await client.post("/api/v1/auth/register", json={"username": "user_one", "password": "Test1234!"})
        token1 = r1.json()["access_token"]
        headers1 = {"Authorization": f"Bearer {token1}"}

        r2 = await client.post("/api/v1/auth/register", json={"username": "user_two", "password": "Test1234!"})
