...
class TestMemoryRepository:
    ...
    async def test_list_all(self):
        ...
        async with session_factory() as session:
            ...
            for i in range(5):
                await repo.create(
                    MemoryCreate(content=f"Memory {i}", category="general"),
                    user_id="user-1",
                )
    ...
    async def test_list_all_filter_by_category(self):
        ...
        async with session_factory() as session:
            ...
            await repo.create(
                MemoryCreate(content="Profile", category="user_profile"),
                user_id="user-1",
            )
    ...
    async def test_list_all_excludes_archived(self):
        ...
        async with session_factory() as session:
            ...
            m1 = await repo.create(
                MemoryCreate(content="Active", category="general"),
                user_id="user-1",
            )
            m2 = await repo.create(
                MemoryCreate(content="Archived", category="general"),
                user_id="user-1",
            )
            m2.is_archived = True
            await session.flush()
    ...
    async def test_update(self):
        ...
        async with session_factory() as session:
            ...
            memory = await repo.create(
                MemoryCreate(content="Original", category="general"),
                user_id="user-1",
            )
    ...
    async def test_update_tags(self):
        ...
        async with session_factory() as session:
            repo = MemoryRepository(session)
            memory = await repo.create(
                MemoryCreate(content="Tag test", tags=["a"]),
                user_id="user-1",
            )
    ...
    async def test_search_by_content_with_category(self):
        ...
        async with session_factory() as session:
            repo = MemoryRepository(session)
            await repo.create(
                MemoryCreate(content="Python rocks", category="general"),
                user_id="user-1",
            )
            await repo.create(
                MemoryCreate(content="Python for ML", category="project"),
                user_id="user-1",
            )
    ...
    async def test_get_by_category(self):
        ...
        async with session_factory() as session:
            repo = MemoryRepository(session)
            await repo.create(
                MemoryCreate(content="Pref: dark mode", category="preference"),
                user_id="user-1",
            )
            await repo.create(
            ...
            await repo.create(
                MemoryCreate(content="General note", category="general"),
                user_id="user-1",
            )
...
class TestMemoryService:
    ...
    async def test_list_filter_by_category(self):
        ...
        async with session_factory() as session:
            service = MemoryService(session)
            await service.create_memory(
                MemoryCreate(content="General", category="general"),
                user_id="user-1",
            )
            await service.create_memory(
                MemoryCreate(content="Pref", category="preference"),
                user_id="user-1",
            )
    ...
    async def test_archive_and_restore(self):
        ...
        async with session_factory() as session:
            service = MemoryService(session)
            created = await service.create_memory(
                MemoryCreate(content="Archivable"),
                user_id="user-1",
            )
    ...
    async def test_get_stats(self):
        ...
        async with session_factory() as session:
            service = MemoryService(session)
            await service.create_memory(
                MemoryCreate(content="A", category="general"),
                user_id="user-1",
            )
            await service.create_memory(
                MemoryCreate(content="B", category="project"),
                user_id="user-1",
            )
            await service.create_memory(
                MemoryCreate(content="C", category="project"),
                user_id="user-1",
            )
...
class TestMemoryAPI:
    ...
    async def test_user_isolation_api(self, client):
        ...
        r1 = await client.post(
            ...
            json={"username": "user_one", "password": "Test1234!"},
        )
        ...
        r2 = await client.post(
            "/api/v1/auth/register",
            json={"username": "user_two", "password": "Test1234!"},
        )
